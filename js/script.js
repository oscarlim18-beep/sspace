document.addEventListener("DOMContentLoaded", function () {
  ...
  if (customToggler && navbarCollapse) {
    customToggler.addEventListener("click", () => {
      customToggler.classList.toggle("is-open");
    });

    navbarCollapse.addEventListener("hidden.bs.collapse", () => {
      customToggler.classList.remove("is-open");
    });
  }
  ...
  // 3D card effect for About page only
  if (card) {
    card.addEventListener("mousemove", (e) => {
      ...
      card.style.boxShadow = `${
        (x - centerX) / 10
      }px ${(y - centerY) / 10}px 40px rgba(0,0,0,0.2)`;
    });
...
// Contact page map switcher
function changeMap(location) {
  const map = document.getElementById("mapFrame");
  const btnKl = document.getElementById("btn-kl");
  const btnKajang = document.getElementById("btn-kajang");

  if (!map || !btnKl || !btnKajang) return;

  if (location === "kl") {
    map.src =
      "https://maps.google.com/maps?q=Residensi%20Aradia%20Kuala%20Lumpur&t=&z=15&ie=UTF8&iwloc=&output=embed";
  ...
  if (location === "kajang") {
    map.src =
      "https://maps.google.com/maps?q=No.199%20Jalan%20Kajang%20Jaya%207%20Taman%20Kajang%20Jaya%20Kajang%20Selangor%2043000&t=&z=17&ie=UTF8&iwloc=&output=embed";
